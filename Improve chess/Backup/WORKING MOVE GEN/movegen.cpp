...
U64 genEnPassantAttacks(U64 generatingPawn, U64 enPassantRights, short SIDE) {
...
U64 genLeftPawnAttack(U64 generatingPawn, short side) {
    U64 move = 0;
...
U64 genRightPawnAttack(U64 generatingPawn, short side) {
    U64 move = 0;
...
U64 genPawnPushes(U64 generatingPawn, U64 &emptySquares, short SIDE) {
    U64 pushes = 0;
...
void convertEnpassantBitboard(short &startSquare, U64 moveBB, MoveList &activeMoveList) {
    // take all the bits out of the move bitboard
    while (moveBB) {
        short endSquare = popIntLSB(moveBB); // get the end square
        // encode the move and add it to the chosen move list
...
void convertQuietPromoBitboard (short &startSquare, U64 moveBB, MoveList &quietMoveList) {
    // take all the bits out of the move bitboard
    while (moveBB) {
        short endSquare = popIntLSB(moveBB); // get the end square
...
void convertActivePromoBitboard(short &startSquare, U64 moveBB, MoveList &activeMoveList, U64 *pieceBB) {
    if (moveBB == 0) return;
...
/* board status stuff */
U64 Position::getSquareAttackers(U64 sq, short SIDE) {
    short mover, enemy;
...
bool Position::checkKingCheck(short SIDE) {
...
short Position::getPieceAt(U64 &sq) {
...
U64 Position::getRay (U64 &from, U64 &to){
    U64 ray;
...
void Position::genQueenMoves() {
    // you very much need to consider blockers for queen moves!

    U64 diagonalBlockers = ~(blockersNE | blockersNW);
    U64 horizontalBlockers = ~(blockersNS | blockersEW);

    U64 generatingPieces = pieceBB[friendly] & pieceBB[QUEEN]; // get the set of queens
...
void Position::genPawnMovesNew() {
    ...
    while (generatingPawns) {
        ...
        short generatingPawnIndex = bitScanForward(generatingPawn);
...
void Position::genPawnMoves() {
    ...
    firstPush &= checkingRay;
    secondPush &= checkingRay;
    while (firstPush) {
    ...
    U64 leftCaptures = shift(capturingPawns & ~(blockersNE), upLeft) & pieceBB[enemy] & checkingRay;
    U64 rightCaptures = shift(capturingPawns & ~(blockersNW), upRight) & pieceBB[enemy] & checkingRay;
    ...
    firstPush = push(pushingPawns, currentSide) & checkingRay & emptySquares;
    while (firstPush) {
        short to = popIntLSB(firstPush);
    ...
    leftCaptures = shift(capturingPawns & ~(blockersNE), upLeft) & pieceBB[enemy] & checkingRay;
    rightCaptures = shift(capturingPawns & ~(blockersNW), upRight) & pieceBB[enemy] & checkingRay;
    for (short piece = PAWN; piece <= KING; piece++) {
        U64 left = leftCaptures & pieceBB[piece];
        U64 right = rightCaptures & pieceBB[piece];
        while (left) {
...
void Position::genCastlingNew() {
    CRights subRights;
    short king, left, right;
    if (currentSide == WHITE) {
        subRights = CstleRights;
        king = E1;
        left = A1;
        right = H1;
    } else {
        subRights = CstleRights >> 2;
        king = E8;
        left = A8;
        right = H8;
    }
    ...
    if ((subRights & 1) &&
        !(CastleMasks[currentSide][0] & occupiedSquares) &&
        !((CastleMasks[currentSide][0] << 1) & attackMap))
    {
        quietMoveList.emplace_back(encodeMove(king, left, 0, 3, KING, EMPTY));
    }

    if ((subRights & 2) &&
        !(CastleMasks[currentSide][1] & occupiedSquares) &&
        !(CastleMasks[currentSide][1] & attackMap))
    {
        quietMoveList.emplace_back(encodeMove(king, right, 0, 3, KING, EMPTY));
    }
}
void Position::genCastlingOld(CastleRights &rights) {
    if (rights.king) {
        // check left castle
        if (rights.left && (!(rights.leftMask & occupiedSquares)) && (!((rights.leftMask << 1) & attackMap))) {
        ...
        // check right castle
        if (rights.right && (!(rights.rightMask & occupiedSquares)) && (!(rights.rightMask  & attackMap))) {
            quietMoveList.emplace_back(
